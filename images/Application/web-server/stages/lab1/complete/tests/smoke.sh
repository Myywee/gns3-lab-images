...
image=${IMAGE_TAG:-gns3-lab/web-server:lab1-complete-smoke}
container=
...
assert_code 403 /secure/
assert_code 403 /.git/HEAD
assert_code 403 /.git/config
assert_code 404 /config.txt
assert_code 404 /definitely-missing
encoded_code=$(docker exec "$container" curl --path-as-is -sS -o /dev/null -w '%{http_code}' \
    http://127.0.0.1/%2egit/config)
case "$encoded_code" in
    2*|3*)
        echo "encoded dotfile path unexpectedly returned HTTP $encoded_code" >&2
        exit 1
        ;;
esac
